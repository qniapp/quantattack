    ...
    describe('swap', function()
      describe('単一ブロック同士の入れ換え', function()
        ...
        it('swap は true を返す', function()
          assert.is_true(board:swap(1, 17))
        end)
        ...
        it("ブロックを入れ換える", function()
          ...
          assert.are_equal("x", board.blocks[1][17].type)
          assert.are_equal("h", board.blocks[2][17].type)
        end)
      end)
      ...
      describe('単一ブロックと I ブロックの入れ換え', function()
        ...
        it("swapping 状態の I ブロックは empty でない", function()
          ...
          assert.is_false(board.blocks[2][17]:is_empty())
        end)
        ...
        it("ブロックを入れ換える", function()
          ...
          assert.are_equal("i", board.blocks[1][17].type)
          assert.are_equal("h", board.blocks[2][17].type)
        end)
      end)
      ...
      describe("プレースホルダブロック (#) の入れ換え", function()
        before_each(function()
          board:put(2, 17, block_class("#"))
        end)

        it("swap は false を返す", function()
          assert.is_false(board:swap(1, 17)) -- 右に # がある場合
          assert.is_false(board:swap(2, 17)) -- 左に # がある場合
        end)
      end)

      describe("おじゃまゲートの入れ換え", function()
        before_each(function()
          board:put(2, 17, garbage_block(3))
        end)

        it("swap は false を返す", function()
          assert.is_false(board:swap(1, 17)) -- 右におじゃまゲートがある場合
          assert.is_false(board:swap(4, 17)) -- 左におじゃまゲートがある場合
        end)
      end)
      ...
      describe("hover 状態のブロックとの入れ替え", function()
        ...
        before_each(function()
          ...
          board:put(1, 17, left_block)
          board:put(2, 17, right_block)
        end)

        it("左のブロックが hover 状態の場合、swap は false を返す", function()
          left_block:hover()

          assert.is_false(board:swap(1, 17))
        end)
      ...
      describe("swapping 状態のブロックとの入れ替え", function()
        ...
        it("左のブロックが swapping 状態の場合、swap は false を返す", function()
          ...
          assert.is_false(board:swap(2, 17))
        end)
      ...
      describe("CNOT の一部と単一ブロックの入れ替え", function()
        before_each(function()
          board:put(1, 17, block_class("h"))
          board:put(2, 17, control_block(3))
          board:put(3, 17, cnot_x_block(2))
          board:put(4, 17, block_class("h"))
        end)

        it("左が CNOT の一部、右が単一ブロックの場合、swap は false を返す", function()
          assert.is_false(board:swap(3, 17))
        end)
      ...
      describe("SWAP の一部と単一ブロックの入れ替え", function()
        before_each(function()
          board:put(1, 17, block_class("h"))
          board:put(2, 17, swap_block(3))
          board:put(3, 17, swap_block(2))
          board:put(4, 17, block_class("h"))
        end)
