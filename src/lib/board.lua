...
local particle, gate, reduction_rules =
require("lib/particle"), require("lib/gate"), require("lib/reduction_rules")
...
function board.put_random_gates(_ENV)
...
function board.reduce_gates(_ENV, game, player, other_board)
  ...
  -- おじゃまゲートのマッチ
  repeat
    local matched_garbage_gate_count = 0
      ...
      if each:is_idle() then
        ...
        local is_matching = function(adjacent_gate)
          ...
          if adjacent_gate.type == "?" then
            if each.body_color == adjacent_gate.body_color then
              each.chain_id = adjacent_gate.chain_id
              return true
            end
          else
            each.chain_id = adjacent_gate.chain_id
        ...
        for gx = x, x + garbage_span - 1 do
          if (y < rows and is_matching(gates[gx][y + 1])) or
            ...
            matched_garbage_gate_count = matched_garbage_gate_count + 1
            goto matched
          end
        end
        ...
        for i = 0, garbage_span - 1 do
          for j = 0, garbage_height - 1 do
            garbage_match_gate = gate("?")
            garbage_match_gate.body_color = each.body_color
  ...
  until matched_garbage_gate_count == 0
end

function board.reduce(_ENV, x, y, include_next_gates)
...
function board._reduce_nocache(_ENV, x, y, include_next_gates)
...
function board._random_single_gate(_ENV)
...
function board.reducible_gate_at(_ENV, x, y)
  return reducible_gates[x][y] or gate("i")
end

function board.put(_ENV, x, y, gate)
  --#if assert
...
function board.remove_gate(_ENV, x, y)
  put(_ENV, x, y, gate("i"))
end
...
function board.is_gate_empty(_ENV, x, y)
  --#if assert
...
function board._gate_or_its_head_gate(_ENV, x, y)
...
-- おじゃまゲート先頭のゲートを返す
-- 一部でない場合は nil を返す
function board._garbage_head_gate(_ENV, x, y)
  for _, each in pairs(_garbage_gates) do
...
-- 一部でない場合は nil を返す
function board._cnot_head_gate(_ENV, x, y)
  for tmp_x = 1, x - 1 do
...
-- SWAP ペア左端のゲートを返す
-- 一部でない場合は nil を返す
function board._swap_head_gate(_ENV, x, y)
  for tmp_x = 1, x - 1 do
    local gate = gates[tmp_x][y]
...
function board.observable_update(_ENV, gate, old_state)
