require("engine/test/bustedhelper")

local i_gate_class = require("i_gate")

describe('i_gate', function()
  local i_gate

  before_each(function()
    i_gate = i_gate_class()
  end)

  describe("gate type", function()
    describe("type", function()
      it("should return 'i'", function()
        assert.are.equals('i', i_gate.type)
      end)
    end)

    describe("is_i", function()
      it("should return true", function()
        assert.is_true(i_gate:is_i())
      end)
    end)

    describe("is_h", function()
      it("should return false", function()
        assert.is_false(i_gate:is_h())
      end)
    end)

    describe("is_x", function()
      it("should return false", function()
        assert.is_false(i_gate:is_x())
      end)
    end)

    describe("is_y", function()
      it("should return false", function()
        assert.is_false(i_gate:is_y())
      end)
    end)

    describe("is_z", function()
      it("should return false", function()
        assert.is_false(i_gate:is_z())
      end)
    end)

    describe("is_s", function()
      it("should return false", function()
        assert.is_false(i_gate:is_s())
      end)
    end)

    describe("is_t", function()
      it("should return false", function()
        assert.is_false(i_gate:is_t())
      end)
    end)

    describe("is_swap", function()
      it("should return false", function()
        assert.is_false(i_gate:is_swap())
      end)
    end)

    describe("is_control", function()
      it("should return false", function()
        assert.is_false(i_gate:is_control())
      end)
    end)

    describe("is_cnot_x", function()
      it("should return false", function()
        assert.is_false(i_gate:is_cnot_x())
      end)
    end)

    describe("is_garbage_gate", function()
      it("should return false", function()
        assert.is_false(i_gate:is_garbage_gate())
      end)
    end)
  end)

  describe("state", function()
    it("should return 'idle'", function()
      assert.are.equals('idle', i_gate.state)
    end)

    describe("is_idle", function()
      it("should return true", function()
        assert.is_true(i_gate:is_idle())
      end)
    end)

    describe("is_busy", function()
      it("should return false", function()
        assert.is_false(i_gate:is_busy())
      end)
    end)

    describe("is_swapping", function()
      it("should return false", function()
        assert.is_false(i_gate:is_swapping())
      end)
    end)

    describe("is_swap_finished", function()
      it("should return false", function()
        assert.is_false(i_gate:is_swap_finished())
      end)
    end)

    describe("is_dropping", function()
      it("should return false", function()
        assert.is_false(i_gate:is_dropping())
      end)
    end)

    describe("is_dropped", function()
      it("should return false", function()
        assert.is_false(i_gate:is_dropped())
      end)
    end)

    describe("is_match", function()
      it("should return false", function()
        assert.is_false(i_gate:is_match())
      end)
    end)
  end)

  describe("dy", function()
    it("should return 0", function()
      assert.are.equals(0, i_gate.dy)
    end)
  end)
end)
