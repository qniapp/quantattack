...
function create_board(_offset_x)
  local board = setmetatable({
    ...
    _garbage_head_gate = function(_ENV, x, y)
      for _, ghead in pairs(garbage_gates) do
        local ghead_x, ghead_y = ghead.x, ghead.y

        if ghead_x <= x and x <= ghead_x + ghead.span - 1 and -- 幅に x が含まれる
